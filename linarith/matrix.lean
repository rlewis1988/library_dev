...
| 0 := []
| (succ n) := list.append (upto n) [n]
end list

namespace fin

definition zero (n : nat) : fin (succ n) := { fin . val := 0, is_lt := zero_lt_succ n }

definition upto (n : nat) : list (fin n) :=
dmap (λ k, k < n) fin.mk (list.upto n)

end fin

def Suml {A B : Type} [add_monoid B] :  list A → (A → B) → B 
| [] f := 0
| [h] f := f h
| (h :: t) f := f h + Suml t f
...
definition r_ith (v : rvector A n) (i : fin n) := v (fin.zero _) i

definition c_ith (v : cvector A m) (i : fin m) := v i (fin.zero _)

definition nonneg [weak_order A] [has_zero A] (M : matrix A m n) := ∀ i j, M i j ≥ 0

definition le [has_le A] (M N : matrix A m n) := ∀ i j, M i j ≤ N i j

definition lt [has_lt A] (M N : matrix A m n) := ∀ i j, M i j < N i j
...
instance matrix_inhabited  [HA : inhabited A] :
        inhabited (M[A]_(succ m, succ n)) :=
  inhabited.rec_on HA (λ a, inhabited.mk (λ s t, a))

open list
/-instance decidable_quant (P : fin n → Prop) [∀ k, decidable (P k)] : decidable (∀ k, P k) :=
  if H : all (fin.upto n) P then
    decidable.true (λ k, of_mem_of_all !fin.mem_upto H)
  else
    decidable.false (λ Hpk, H (all_of_forall (λ a Ha, Hpk a)))
...
/-instance m_left_module  [reducible] (A : Type) [ring A] (m n : ℕ) : left_module A (matrix A m n) :=
  begin
